...
namespace Chat.Desktop
{
    public class ChatRoomForm : Form
    {
        ...
        public ChatRoomForm(ApiService api, ChatHubService hub, string userId, int roomId, string roomName)
        {
            _api = api;
            _hub = hub;
        ...
        {
            base.OnLoad(e);
            _hub.RoomMessageReceived += OnRoomMessageReceived;
        ...
        {
            _hub.RoomMessageReceived -= OnRoomMessageReceived;
            await _hub.LeaveRoomAsync(_roomId);
            base.OnFormClosed(e);
        }

        private async Task LoadHistoryAsync()
        {
            var messages = await _api.GetAsync<List<JsonElement>>($"api/messages/room/{_roomId}");
        ...
        private void OnRoomMessageReceived(RoomMessageDto msg)
        {
            if (IsDisposed) return;
            if (InvokeRequired) { BeginInvoke(() => OnRoomMessageReceived(msg)); return; }
        ...
        private async void SendMessage()
        {
            ...
            txtMessage.Clear();
            await _hub.SendRoomMessageAsync(_roomId, text);
        }
    }
}
