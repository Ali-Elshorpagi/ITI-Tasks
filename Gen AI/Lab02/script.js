...
const THEME_KEY = "chatgpt-ui-clone-theme-v1";
...
function renderChatList(popChatId = "") {
  ...
  if (chatsCountEl) {
    chatsCountEl.textContent = `Chats: ${state.chats.length}`;
  }
}
...
function renderMessage(message, options = {}) {
  ...
  if (message.role === "assistant" && !message.isNotice) {
    const metaEl = document.createElement("div");
    metaEl.className = "msg-meta";
    metaEl.textContent = "Hera response";
  ...
  if (message.role === "assistant" && message.type === "text" && message.text) {
    const actionsEl = document.createElement("div");
...
function renderMessages(stagger = false) {
  ...
  if (!activeChat || activeChat.messages.length === 0) {
    ...
    emptyState.innerHTML =
      '<h2>Start a conversation</h2><p>Type a question below or tap a quick prompt to get started faster.</p>';
...
function setSendingStatus(sending) {
  ...
  if (regenBtn) {
    regenBtn.disabled = sending;
  }
  if (editLastBtn) {
    editLastBtn.disabled = sending;
  }
  if (deleteLastBtn) {
    deleteLastBtn.disabled = sending;
  }
...
function updatePromptActionButtons() {
  const activeChat = getActiveChat();
  const hasUserPrompt = Boolean(activeChat && findLastUserMessageIndex(activeChat.messages) !== -1);
  const disabled = isSending || !hasUserPrompt;
  ...
  if (deleteLastBtn) {
    deleteLastBtn.disabled = disabled;
  }
}
...
async function handleRegenerateLast() {
  ...
  const activeChat = getActiveChat();
  const lastUserIndex = findLastUserMessageIndex(activeChat.messages);
  if (lastUserIndex === -1) {
    return;
  }

  const nextMessage = activeChat.messages[lastUserIndex + 1];
  if (nextMessage && nextMessage.role === "assistant") {
    activeChat.messages.splice(lastUserIndex + 1, 1);
  }
...
function handleEditLastPrompt() {
  ...
  const lastUserMessage = activeChat.messages[lastUserIndex];
  inputEl.value = lastUserMessage.text || "";

  activeChat.messages.splice(lastUserIndex);
  saveState();
  renderMessages(false);
  updatePromptActionButtons();
  inputEl.focus();
...
function handleDeleteLastPrompt() {
  if (isSending) {
    return;
  }
...
if (regenBtn) {
  regenBtn.addEventListener("click", handleRegenerateLast);
}
if (editLastBtn) {
  editLastBtn.addEventListener("click", handleEditLastPrompt);
}
if (deleteLastBtn) {
  deleteLastBtn.addEventListener("click", handleDeleteLastPrompt);
}
if (themeToggleBtn) {
